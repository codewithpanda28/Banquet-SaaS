...
export default function SuperAdminPage() {
    ...
    const [activeTab, setActiveTab] = useState<'restaurants' | 'analytics' | 'support' | 'settings'>('restaurants');
    ...
    const handleNodeSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (editingId) {
                ...
                if (error) throw error;
                toast.success('New restaurant added to platform');
            }
            setIsModalOpen(false);
            setEditingId(null);
            setFormData({ name: '', slug: '', custom_domain: '', admin_passcode: '', logo_url: '' });
            fetchRestaurants();
        } catch (error: any) {
    ...
    const fetchGlobalStats = useCallback(async () => {
        try {
            ...
            if (orders) {
                ...
                const sortedNodes = Object.entries(nodeSales).map(([id, revenue]) => ({ id, revenue: revenue as number, name: restaurants.find(r => r.id === id)?.name || 'Unknown' })).sort((a, b) => b.revenue - a.revenue).slice(0, 5);
    ...
    const handleUpdateTicket = async (id: string, status: string) => {
        ...
        if (!error) toast.success(`Ticket marked as ${status.toUpperCase()}`);
    ...
    const handleDeleteTicket = async (id: string) => {
        ...
        if (error) { toast.error('Delete Failed'); fetchTickets(); }
    };
        ...
        return (
            <div className="min-h-screen bg-[#0A0A0A] flex items-center justify-center p-6 text-white text-center">
                <div className="w-full max-w-md bg-[#111111] border border-white/10 p-12 rounded-[4rem] relative overflow-hidden">
                    <div className="absolute inset-0 bg-blue-600/10 blur-[90px]" />
                    <ShieldCheck className="text-blue-500 w-16 h-16 mb-8 mx-auto relative z-10" />
                    <h1 className="text-4xl font-black italic mb-8 relative z-10">Super Admin</h1>
                    <form onSubmit={handleAuth} className="space-y-6 relative z-10">
                        ...
                        <button type="submit" className="w-full bg-white text-black font-black py-6 rounded-[2rem] uppercase tracking-widest text-[10px] hover:scale-105 transition-all">Unlock Dashboard</button>
                    </form>
                </div>
            </div>
        )
        ...
        <div className="min-h-screen bg-[#0A0A0A] text-white flex overflow-hidden">
            ...
            <div className="w-20 lg:w-72 border-r border-white/5 flex flex-col py-10 px-4 bg-[#0A0A0A] z-40">
                <div className="flex items-center gap-3 mb-16 px-4">
                    <div className="w-10 h-10 bg-blue-600 rounded-xl flex items-center justify-center"><Zap size={24} /></div>
                    <span className="text-xl font-bold italic hidden lg:block tracking-tight uppercase">Banquet Admin</span>
                </div>
                <nav className="flex-1 space-y-3">
                    {[
                        { id: 'restaurants', label: 'Restaurants', icon: <LayoutDashboard size={20} /> },
                ...
                <div className="max-w-7xl mx-auto pb-40">
                    {activeTab === 'restaurants' && (
                        <div className="space-y-12 animate-in fade-in duration-500">
                            <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                                <div><h1 className="text-3xl font-black italic tracking-tighter mb-1">Restaurants</h1><p className="text-[10px] text-blue-500 font-bold uppercase tracking-widest">{restaurants.length} active units registered</p></div>
                                <div className="flex gap-3">
                                    <input type="text" placeholder="Search restaurant..." className="bg-white/5 border border-white/10 rounded-xl py-3 px-6 w-48 text-sm focus:ring-1 focus:ring-blue-500 outline-none" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                                    <button onClick={() => { setEditingId(null); setFormData({ name: '', slug: '', custom_domain: '', admin_passcode: '', logo_url: '' }); setIsModalOpen(true); }} className="bg-white text-black font-black px-6 py-3 rounded-xl flex items-center gap-2 text-xs transition-all shadow-xl hover:scale-105"><Plus size={16} /> Add Restaurant</button>
                                </div>
                            </header>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {restaurants.filter(r => r.name?.toLowerCase().includes(searchTerm.toLowerCase())).map(restaurant => (
                                    <div key={restaurant.id} className="bg-[#111111] border border-white/5 p-6 rounded-3xl hover:border-blue-500/40 transition-all duration-500 flex flex-col justify-between group h-full relative overflow-hidden">
                                        <div>
                                            ...
                                            <h3 className="text-xl font-black mb-1 tracking-tight group-hover:text-blue-400 uppercase italic">{restaurant.name}</h3>
                                        ...
                                        <div className="flex gap-2 mt-6 relative z-10">
                                            <button onClick={() => setSelectedRestro(restaurant)} className="flex-1 bg-blue-600 font-black py-3 rounded-xl text-[9px] uppercase shadow-lg text-white hover:bg-blue-500 transition-all">Quick Links</button>
                                            <button onClick={() => { setEditingId(restaurant.id); setFormData({ name: restaurant.name || '', slug: restaurant.slug || '', custom_domain: restaurant.custom_domain || '', admin_passcode: restaurant.admin_passcode || '', logo_url: restaurant.logo_url || '' }); setIsModalOpen(true); }} className="px-4 bg-white/5 hover:bg-white/10 rounded-xl border border-white/5 transition-all"><Pencil size={14} className="text-gray-400" /></button>
                                            <button
                                                type="button"
                                                onClick={(e) => {
                                                    e.stopPropagation();
                    ...
                    {activeTab === 'support' && (
                        <div className="space-y-12 animate-in slide-in-from-right-10 duration-700">
                            <header><h1 className="text-5xl font-black italic tracking-tighter">Support</h1><p className="text-xs text-rose-500 font-bold uppercase tracking-widest mt-2 px-1 border-l-2 border-rose-500 ml-1">Client Issues & Feedback</p></header>
                                ...
                                {tickets.length > 0 ? tickets.map(t => (
                                    <div key={t.id} className={cn("bg-[#111111] border p-8 rounded-[3.5rem] space-y-6 group transition-all duration-700 relative overflow-hidden", t.status === 'resolved' ? 'border-emerald-500/20 opacity-40 translate-y-4' : 'border-rose-500/20 shadow-2xl shadow-rose-500/5')}>
                                        <div className="flex justify-between items-start relative z-10">
                                            <div className="px-5 py-2 bg-blue-500/10 rounded-full text-[9px] font-black uppercase tracking-widest text-blue-400 border border-white/5">{t.restaurants?.name}</div>
                                        ...
                                        {t.image_url && (
                                            <div onClick={() => setPreviewImage(t.image_url)} className="relative h-60 w-full rounded-[2rem] overflow-hidden border border-white/5 bg-gray-950 cursor-pointer group/img">
                                                ...
                                                <div className="absolute inset-x-0 bottom-0 bg-black/80 opacity-0 group-hover/img:opacity-100 flex items-center justify-center transition-opacity py-3">
                                                    <div className="flex items-center gap-2 text-white font-black text-[10px] uppercase tracking-tighter"><Maximize2 size={12} /> View Full Image</div>
                                                </div>
                                            </div>
                                        )}
                                        ...
                                        <div className="flex gap-2 border-t border-white/5 pt-6 relative z-10">
                                            {t.status === 'open' ? (
                                                <button onClick={() => handleUpdateTicket(t.id, 'resolved')} className="flex-1 py-4 bg-white text-black rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-xl hover:scale-105 transition-all">Mark Resolved</button>
                                            ) : (
                                                <button onClick={() => handleUpdateTicket(t.id, 'open')} className="flex-1 py-4 bg-white/5 text-gray-500 rounded-2xl font-black uppercase text-[10px] tracking-widest border border-white/5 hover:text-white transition-all">Re-Open</button>
                                            )}
                                    ...
                                    <div className="col-span-full py-40 text-center flex flex-col items-center opacity-20"><CheckCircle2 size={60} className="mb-4 text-emerald-500" /><h3 className="text-2xl font-black italic uppercase tracking-[0.5em]">No Support Tickets</h3></div>
                                )}
                            </div>
                        </div>
                    )}
                    ...
                    {activeTab === 'analytics' && (
                        <div className="space-y-12 animate-in fade-in zoom-in duration-700">
                            <header>
                                <h1 className="text-5xl font-black italic tracking-tighter">Analytics</h1>
                                <p className="text-xs text-indigo-500 font-bold uppercase tracking-widest mt-2 px-1 border-l-2 border-indigo-500 ml-1">Real-time Performance Metrics</p>
                            </header>
                            ...
                            <div className="bg-[#111111] border border-white/5 p-12 rounded-[4rem]">
                                <h3 className="text-xl font-black italic uppercase tracking-widest mb-10 text-gray-400">Revenue Trends</h3>
                            ...
                            <div className="bg-[#111111] border border-white/5 p-12 rounded-[4rem]">
                                <h3 className="text-xl font-black italic uppercase tracking-widest mb-10 text-gray-400">Top Restaurants</h3>
                                    ...
                                    {globalStats.topNodes.length > 0 ? globalStats.topNodes.map((node, i) => (
                                        <div key={i} className="flex items-center justify-between p-6 bg-white/[0.02] border border-white/5 rounded-[2rem] hover:bg-white/[0.05] transition-all">
                                            <div className="flex items-center gap-6">
                                                <div className="w-12 h-12 bg-blue-600 rounded-xl flex items-center justify-center font-black">{i + 1}</div>
                                                <div>
                                                    <h4 className="text-lg font-bold italic uppercase">{node.name}</h4>
                                                    <p className="text-[10px] text-gray-500 uppercase tracking-widest">ID: {node.id.slice(0, 8)}...</p>
                                                </div>
                                            </div>
                                            <div className="text-right">
                                                <p className="text-xl font-black italic text-emerald-500">₹{node.revenue.toLocaleString()}</p>
                                                <p className="text-[10px] text-gray-500 uppercase font-black">Total Sales</p>
                                            </div>
                                        </div>
                                    )) : (
                                        <div className="text-center py-20 opacity-20"><Database size={40} className="mx-auto mb-4" /><p className="uppercase tracking-[0.5em] font-black">No Data Available</p></div>
                                    )}
                                </div>
                            </div>
                        </div>
                    ...
                    {activeTab === 'settings' && (
                        <div className="space-y-12 animate-in slide-in-from-bottom-10 duration-700">
                            <header>
                                ...
                                <p className="text-xs text-orange-500 font-bold uppercase tracking-widest mt-2 px-1 border-l-2 border-orange-500 ml-1">Platform-Wide Configuration</p>
                            </header>

                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="bg-[#111111] border border-white/5 p-12 rounded-[4rem] space-y-8">
                                    <h3 className="text-xl font-black italic uppercase tracking-widest text-gray-400">Brand Identity</h3>
                                    <div className="space-y-6">
                                        <div className="space-y-2">
                                            <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest">App Name</label>
                                            <input type="text" defaultValue="Banquet Manager" className="w-full bg-black border border-white/10 p-6 rounded-[2rem] outline-none focus:border-blue-500 transition-all font-bold italic" />
                                        </div>
                                        <div className="space-y-2">
                                            <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Support Contact</label>
                                            <input type="text" defaultValue="+91 1234567890" className="w-full bg-black border border-white/10 p-6 rounded-[2rem] outline-none focus:border-blue-500 transition-all font-bold italic" />
                                        </div>
                                    </div>
                                </div>

                                <div className="bg-[#111111] border border-white/5 p-12 rounded-[4rem] space-y-8">
                                    <h3 className="text-xl font-black italic uppercase tracking-widest text-gray-400">Control Panel</h3>
                                    <div className="space-y-6">
                                        <div className="flex items-center justify-between p-6 bg-white/[0.02] border border-white/5 rounded-[2.5rem]">
                                            <div>
                                                ...
                                                <p className="text-[9px] text-gray-500 uppercase tracking-widest">Disables public access</p>
                                            </div>
                                        ...
                                        <div className="flex items-center justify-between p-6 bg-white/[0.02] border border-white/5 rounded-[2.5rem]">
                                            <div>
                                                <p className="font-black italic uppercase text-xs">New Registrations</p>
                                                <p className="text-[9px] text-gray-500 uppercase tracking-widest">Allow new signups</p>
                                            </div>
                                ...
                                <div className="col-span-full bg-white/5 border border-white/10 p-12 rounded-[4rem] text-center">
                                    <button onClick={() => toast.success('Settings Saved')} className="bg-white text-black font-black px-16 py-6 rounded-[2.5rem] uppercase tracking-[0.2em] text-[10px] hover:scale-105 transition-all shadow-2xl">Save Changes</button>
                                </div>
                            </div>
                        </div>
                    )}
            ...
            {/* Image Preview */}
            {previewImage && (
                <div className="fixed inset-0 z-[150] bg-black/98 p-10 flex items-center justify-center animate-in fade-in duration-300" onClick={() => setPreviewImage(null)}>
                    ...
                    <div className="max-w-[90vw] max-h-[85vh] relative rounded-[4rem] overflow-hidden border border-white/10" onClick={e => e.stopPropagation()}>
                ...
                <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/95 p-4 backdrop-blur-xl">
                    <div className="bg-[#111111] border border-white/10 w-full max-w-4xl p-10 md:p-14 rounded-[3rem] shadow-4xl relative overflow-hidden">
                        ...
                        <div className="text-center mb-10 relative">
                            ...
                            <h2 className="text-4xl font-black italic uppercase tracking-tighter">{selectedRestro.name} <span className="text-gray-500">Access</span></h2>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 relative z-10">
                            {[
                                { label: 'Admin Dashboard', icon: <Settings />, color: 'bg-blue-600', href: getFullLink('/admin', selectedRestro) },
                                { label: 'Kitchen Display', icon: <Utensils />, color: 'bg-orange-600', href: getFullLink('/kitchen', selectedRestro) },
                                { label: 'Waiter App', icon: <Zap />, color: 'bg-indigo-600', href: getFullLink('/waiter', selectedRestro) },
                                { label: 'Customer Menu', icon: <Globe />, color: 'bg-emerald-600', href: getFullLink('/customer/menu', selectedRestro) }
                            ].map((l, i) => (
                ...
                <div className="fixed inset-0 z-[150] flex items-center justify-center bg-black/95 p-4 backdrop-blur-xl">
                    <div className="bg-[#111111] border border-white/10 w-full max-w-4xl p-8 md:p-12 rounded-[2.5rem] shadow-4xl relative max-h-[95vh] overflow-y-auto custom-scrollbar">
                        <div className="flex justify-between items-center mb-8">
                            <h2 className="text-2xl font-black italic uppercase tracking-tighter">{editingId ? 'Edit Restaurant' : 'Add New Restaurant'}</h2>
                        ...
                        <form onSubmit={handleNodeSubmit} className="space-y-8">
                            <div className="space-y-4">
                                <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-blue-500 border-b border-white/5 pb-2">Basic Information</h3>
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <div className="space-y-1">
                                        <label className="text-[9px] uppercase font-black text-gray-500 ml-2">Restaurant Name</label>
                                        <input type="text" placeholder="The Gold Biryani" className="w-full bg-black border border-white/10 p-4 rounded-xl outline-none focus:border-blue-500 transition-all text-sm" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} required />
                                    </div>
                                    <div className="space-y-1">
                                        <label className="text-[9px] uppercase font-black text-gray-500 ml-2">Slug</label>
                                        <input type="text" placeholder="gold-biryani" className="w-full bg-black border border-white/10 p-4 rounded-xl outline-none focus:border-blue-500 transition-all text-sm" value={formData.slug} onChange={(e) => setFormData({ ...formData, slug: e.target.value })} required />
                                    </div>
                                    <div className="space-y-1">
                                        <label className="text-[9px] uppercase font-black text-gray-500 ml-2">Custom Domain</label>
                                        <input type="text" placeholder="domain.com" className="w-full bg-black border border-white/10 p-4 rounded-xl outline-none focus:border-blue-500 transition-all text-sm" value={formData.custom_domain} onChange={(e) => setFormData({ ...formData, custom_domain: e.target.value })} />
                                    </div>
                                </div>
                            </div>

                            <div className="space-y-4">
                                <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-indigo-500 border-b border-white/5 pb-2">Visuals & Billing</h3>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="space-y-1">
                                        <label className="text-[9px] uppercase font-black text-gray-500 ml-2">Admin Passcode</label>
                                        <input type="text" maxLength={4} placeholder="1234" className="w-full bg-black border border-white/10 p-4 rounded-xl outline-none focus:border-blue-500 transition-all text-sm font-mono tracking-widest" value={formData.admin_passcode} onChange={(e) => setFormData({ ...formData, admin_passcode: e.target.value })} required />
                                    </div>
                                    <div className="space-y-1">
                                        <label className="text-[9px] uppercase font-black text-gray-500 ml-2">Logo URL</label>
                                        <input type="text" className="w-full bg-black border border-white/10 p-4 rounded-xl outline-none focus:border-blue-500 transition-all text-sm" value={formData.logo_url} onChange={(e) => setFormData({ ...formData, logo_url: e.target.value })} />
                                    </div>
                                </div>
                            </div>

                            <div className="pt-6">
                                <button type="submit" disabled={isLoading} className="w-full bg-white text-black font-black py-5 rounded-xl uppercase tracking-[0.2em] text-[10px] hover:scale-[1.01] transition-all flex items-center justify-center gap-3">
                                    {isLoading ? <Loader2 className="animate-spin" size={16} /> : (editingId ? 'Save Changes' : 'Register Restaurant')}
                                </button>
                            </div>
                        </form>
                    </div>
            ...
            {deleteConfirmId && (
                <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/98 p-4 backdrop-blur-3xl">
                    <div className="bg-[#111111] border border-white/10 w-full max-w-md p-10 rounded-[3rem] shadow-4xl text-center relative overflow-hidden">
                        ...
                        <div className="w-20 h-20 bg-rose-600/20 rounded-3xl flex items-center justify-center mx-auto mb-8 border border-rose-500/20">
                            <Trash2 className="text-rose-500 w-10 h-10" />
                        </div>
                        <h2 className="text-2xl font-black italic uppercase tracking-tighter mb-4">Delete Restaurant?</h2>
                        <p className="text-gray-500 text-xs leading-relaxed mb-10 px-4 uppercase tracking-widest font-bold opacity-60">
                            Are you sure you want to remove <span className="text-rose-500 font-black italic underline">"{deleteConfirmName}"</span>? This action cannot be undone.
                        </p>
                        <div className="flex flex-col gap-3 relative z-10">
                            <button
                                ...
                                disabled={isLoading}
                                className="w-full py-5 bg-rose-600 text-white font-black rounded-2xl uppercase tracking-widest text-[10px] hover:bg-rose-500 transition-all"
                            >
                                {isLoading ? 'Deleting...' : 'Confirm Delete'}
                            </button>
                            <button onClick={() => setDeleteConfirmId(null)} className="w-full py-5 bg-white/5 text-gray-400 font-extrabold rounded-2xl uppercase tracking-widest text-[10px] hover:text-white transition-all border border-white/10">Cancel</button>
                        </div>
                    </div>
                </div>
            )}
